...
def fieldOfView(startX, startY, mapWidth, mapHeight, radius, funcVisitTile, funcTileBlocked):
    """
    ...
    # Northeast quadrant
    __checkQuadrant(visited, startX, startY, 1, 1, maxExtentX, maxExtentY, funcVisitTile, funcTileBlocked)

    # Southeast quadrant
    __checkQuadrant(visited, startX, startY, 1, -1, maxExtentX, minExtentY, funcVisitTile, funcTileBlocked)

    # Southwest quadrant
    __checkQuadrant(visited, startX, startY, -1, -1, minExtentX, minExtentY, funcVisitTile, funcTileBlocked)

    # Northwest quadrant
    __checkQuadrant(visited, startX, startY, -1, 1, minExtentX, maxExtentY, funcVisitTile, funcTileBlocked)
...
class __Line(object):
        ...
        def lineCollinear(self, line):
            return self.pCollinear(line.xi, line.yi) and self.pCollinear(line.xf, line.yf)
...
def __checkQuadrant(visited, startX, startY, dx, dy, extentX, extentY, funcVisitTile, funcTileBlocked):
    ...
    while i != maxI + 1 and len(activeViews) > 0:
        ...
        while j != maxJ + 1 and viewIndex < len(activeViews):
            x = i - j
            y = j
            __visitCoord(visited, startX, startY, x, y, dx, dy, viewIndex, activeViews, funcVisitTile, funcTileBlocked)

            j += 1

        i += 1

def __visitCoord(visited, startX, startY, x, y, dx, dy, viewIndex, activeViews, funcVisitTile, funcTileBlocked):
    # The top left and bottom right corners of the current coordinate.
    topLeft = (x, y + 1)
    bottomRight = (x + 1, y)

    while viewIndex < len(activeViews) and activeViews[viewIndex].steepLine.pBelowOrCollinear(bottomRight[0], bottomRight[1]):
        # The current coordinate is above the current view and is
        # ignored.  The steeper fields may need it though.
        viewIndex += 1

    if viewIndex == len(activeViews) or activeViews[viewIndex].shallowLine.pAboveOrCollinear( topLeft[0], topLeft[1]):
    ...
    if activeViews[viewIndex].shallowLine.pAbove( bottomRight[0], bottomRight[1]) and activeViews[viewIndex].steepLine.pBelow( topLeft[0], topLeft[1]):
        ...
        __addShallowBump(topLeft[0], topLeft[1], activeViews, steepViewIndex)
...
def __addShallowBump(x, y, activeViews, viewIndex):
    ...
    activeViews[viewIndex].shallowBump = __ViewBump(x, y, activeViews[viewIndex].shallowBump)
...
def __addSteepBump(x, y, activeViews, viewIndex):
    ...
    activeViews[viewIndex].steepBump = __ViewBump(x, y, activeViews[viewIndex].steepBump)
