...
class Map:
    ...
    #     self.level = self.tunnelingAlgorithm.generateLevel(self.map_width, self.map_height)
    #     self._previousGenerator = self.tunnelingAlgorithm
    #
    # def useBSPTree(self):
    #     self.level = self.bspTree.generateLevel(self.map_width, self.map_height)
    #     self._previousGenerator = self.bspTree
    #
    # def useDrunkardsWalk(self):
    #     self.level = self.drunkardsWalk.generateLevel(self.map_width, self.map_height)
    #     self._previousGenerator = self.drunkardsWalk

    def useCellularAutomata(self):
        self.level = self.cellularAutomata.generateLevel(self.map_width, self.map_height)
        self._previousGenerator = self.cellularAutomata
    #
    # def useRoomAddition(self):
    #     self.level = self.roomAddition.generateLevel(self.map_width, self.map_height)
    #     self._previousGenerator = self.roomAddition
    #
    # def useCityWalls(self):
    #     self.level = self.cityWalls.generateLevel(self.map_width, self.map_height)
    #     self._previousGenerator = self.cityWalls
    #
    # def useMazeWithRooms(self):
    #     self.level = self.mazeWithRooms.generateLevel(self.map_width,self.map_height)
    #     self._previousGenerator = self.mazeWithRooms
    #
    # def useMessyBSPTree(self):
    #     self.level = self.messyBSPTree.generateLevel(self.map_width,self.map_height)
    ...
    def make_map_dict(self):
        ...
        for x in range(self.map_width):
            for y in range(self.map_height):
                self.mdict[x,y] = [self.level[x][y],None]
        return self.mdict

    def freeSpaces(self):
        freespace = []
        for xy in list(self.mdict.keys()):
            # print (xy)
            if self.mdict[xy][0] != 1:
                freespace.append(xy)
        return freespace
